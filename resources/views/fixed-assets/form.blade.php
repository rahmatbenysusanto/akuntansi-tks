...
<div class="max-w-2xl bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
    <form method="POST" data-confirm="Yakin ingin menyimpan aset tetap ini?">@csrf
        ...
        <div class="grid grid-cols-3 gap-4 mt-4">
            <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Akun Aset</label>
                <select name="account_id" class="w-full rounded-lg input-modern text-sm account-select" required>@foreach($assetAccounts as $a)<option value="{{ $a->id }}">{{ $a->code }} - {{ $a->name }}</option>@endforeach</select></div>
            <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Akun Akum. Penyusutan</label>
                <select name="accumulated_depreciation_account_id" class="w-full rounded-lg input-modern text-sm account-select" required>@foreach($assetAccounts as $a)<option value="{{ $a->id }}">{{ $a->code }} - {{ $a->name }}</option>@endforeach</select></div>
            <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Akun Beban Penyusutan</label>
                <select name="depreciation_expense_account_id" class="w-full rounded-lg input-modern text-sm account-select" required>@foreach($assetAccounts as $a)<option value="{{ $a->id }}">{{ $a->code }} - {{ $a->name }}</option>@endforeach</select></div>
        </div>
