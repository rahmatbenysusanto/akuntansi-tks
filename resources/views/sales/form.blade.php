<x-app-layout>
<x-slot name="header">Sales Invoice Baru</x-slot>
<div class="max-w-4xl bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
    <form method="POST" action="{{ route('sales.store') }}">@csrf
        <div class="grid grid-cols-3 gap-4">
            <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Customer</label>
                <select name="customer_id" class="w-full rounded-lg input-modern text-sm" required>@foreach($customers as $c)<option value="{{ $c->id }}">{{ $c->name }}</option>@endforeach</select></div>
