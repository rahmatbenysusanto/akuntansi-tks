...
<div class="space-y-4">
    <div class="flex justify-end">
        <a href="{{ route('purchases.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">+ Purchase Invoice Baru</a>
    </div>
    <div class="bg-white rounded-lg shadow-sm border overflow-x-auto">
        <table class="w-full text-sm">
            <thead><tr class="text-left text-gray-500 border-b bg-gray-50">
                <th class="px-4 py-3">Invoice#</th><th class="px-4 py-3">Vendor</th>
            ...
            @forelse($invoices as $inv)
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2">{{ $inv->vendor->name ?? '-' }}</td>
