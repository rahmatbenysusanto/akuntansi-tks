<x-app-layout>
<x-slot name="header">Aging Piutang</x-slot>
<div class="bg-white rounded-lg shadow-sm border">
    <table class="w-full text-sm">
        <thead><tr class="text-left text-gray-500 border-b bg-gray-50">
            <th class="px-4 py-3">Customer</th><th class="px-4 py-3 text-right">0-30 Hari</th>
        ...
        @forelse($aging as $a)
            <tr class="border-b hover:bg-gray-50">
                <td class="px-4 py-2">{{ $a['customer']->name }}</td>
                <td class="px-4 py-2 text-right {{ $a['aging']['0-30'] > 0 ? 'text-green-600' : '' }}">{{ number_format($a['aging']['0-30'], 0, ',', '.') }}</td>
                <td class="px-4 py-2 text-right {{ $a['aging']['31-60'] > 0 ? 'text-amber-600' : '' }}">{{ number_format($a['aging']['31-60'], 0, ',', '.') }}</td>
                <td class="px-4 py-2 text-right {{ $a['aging']['61-90'] > 0 ? 'text-orange-600' : '' }}">{{ number_format($a['aging']['61-90'], 0, ',', '.') }}</td>
                <td class="px-4 py-2 text-right {{ $a['aging']['>90'] > 0 ? 'text-red-600 font-bold' : '' }}">{{ number_format($a['aging']['>90'], 0, ',', '.') }}</td>
                <td class="px-4 py-2 text-right font-semibold">{{ number_format($a['total'], 0, ',', '.') }}</td>
            </tr>
        @empty<tr><td colspan="6" class="px-4 py-8 text-center text-gray-400">Tidak ada piutang outstanding.</td></tr>
        @endforelse
        </tbody>
