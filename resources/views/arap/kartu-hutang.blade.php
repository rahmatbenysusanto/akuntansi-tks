<x-app-layout>
<x-slot name="header">Kartu Hutang - Vendor</x-slot>
<div class="space-y-4">
    <div class="bg-white rounded-lg shadow-sm border p-5">
        <form method="GET" class="flex gap-3 items-end">
            <div><label class="text-sm text-gray-600 mb-1 block">Pilih Vendor</label>
                <select name="vendor_id" class="rounded-lg border-gray-300 text-sm" onchange="this.form.submit()">
                    <option value="">-- Pilih --</option>
                    @foreach($vendors as $v)
                        <option value="{{ $v->id }}" {{ ($selectedVendor->id ?? '') == $v->id ? 'selected' : '' }}>{{ $v->name }}</option>
                    @endforeach
                </select></div>
        </form>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border">
        <div class="px-5 py-4 border-b flex justify-between">
            <div><h3 class="font-semibold">{{ $data['vendor']->name }}</h3></div>
            <div class="text-right"><p class="text-sm text-gray-500">Saldo Hutang</p><p class="text-xl font-bold text-red-600">{{ number_format($data['saldo'], 0, ',', '.') }}</p></div>
        </div>
